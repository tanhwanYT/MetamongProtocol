...
public class Gen_6947eb44_e2b2_4f44_a03c_099e1da29d55 : MonoBehaviour
{
    // === User Variables ===
    public int player = 0;
    ...
    void Update()
    {
        ...
        if (hp > 0)
        {
            ...
            if (_animator != null)
            {
                ...
                {
                    Debug.LogError($"[Action] PlayAnimation Failed: No AnimatorController assigned/ found on 'SkeletonWarriorMove_default' request.");
                }
                else
                {
                    Debug.Log($"[Action] PlayAnimationRequest: 'SkeletonWarriorMove_default' on {_animator.gameObject.name} (Controller: {_animator.runtimeAnimatorController.name})");
                    _animator.Play("SkeletonWarriorMove_default");
                }
            }
        ...
        if (hpp <= 0f)
        {
            ...
            Debug.Log("[Action] PlayParticle: blood");
            ParticleManager.PlayStatic("blood", _transform.position, 1f);
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            hpp = hpp - damage;
            // RunModule: b7183f66-61d8-4f28-91d0-10d6fed24806
            AudioManager.PlayStatic("");
        }
        if (true)
        {
